...
app_main_loop_worker(void) {
	...
	while (!force_quit) {
		for (i = 0; i < app.n_ports; i++) {
			...
			RTE_LOG(INFO, USER1, "WORKER: Dequeued %u packets from RX ring %u\n",
            app.burst_size_worker_read, i);
			...
			do {
				...
				if (ret != 0)
                	RTE_LOG(INFO, USER1, "WORKER: Enqueued %u packets to TX ring %u\n",
                    app.burst_size_worker_write, i);
			} while (ret == 0 && !force_quit);
		}
	}
}
...
app_main_loop_tx(void) {
	...
	while (!force_quit) {
		for (i = 0; i < app.n_ports; i++) {
			...
			tx_count += n_pkts;
			RTE_LOG(INFO, USER1, "TX: Port %u sent %u packets (Total TX: %"PRIu64 ")\n", 
				app.ports[i], n_pkts, tx_count);
